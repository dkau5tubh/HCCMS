...
public class UpdateProfile extends JFrame {
	...
	public UpdateProfile() {
		...
		label_9.setText(AccountantHomeScreen.username);
		panel.add(label_9);
		...
		btnBack.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				AccountantHomeScreen accountantHomeScreen=new AccountantHomeScreen();
				accountantHomeScreen.setVisible(true);
			}
		});
		btnBack.setBackground(SystemColor.textHighlight);
		btnBack.setIcon(new ImageIcon("J:\\Hospital Management Swing\\hospital\\icons8-left-64.png"));
		...
		lblPassword.setBounds(443, 463, 112, 16);
		panel_2.add(lblPassword);
		
		textField_18 = new JTextField();
		textField_18.setColumns(10);
		textField_18.setBounds(525, 467, 181, 26);
		panel_2.add(textField_18);
		
		try {
			Connection connection=DBManager.getDBConnectivity();
			Statement statement=connection.createStatement();
			String id= AccountantHomeScreen.username;
	...
	public void getUpdate() {
		...
		try {
			...
			String sql = "update employees set  role=?,gender=?,dob=?,qualification=?,salary=?,"
					+ "email=?,mobno=?,department=?,bloodgrp=?,shift=?,password=? where id='" + id+ "'";
			...
			statement.setString(11, textField_18.getText());
			statement.executeUpdate();
			//System.out.println("data updated sucssesfully");

			///--------------*** Update into Userd table ***------------------
			String updateINTOUsers="update USERS set PASSWORD=? where id='"+id+"' ";
			statement=connection.prepareStatement(updateINTOUsers);
			statement.setString(1,textField_18.getText());
			//--------------*** End Update into Userd table ***------------------
			statement.executeUpdate();
			System.out.println("data updated sucssesfully");
